...
float light(float d, float range, float energy)
{
	float ld = d / range;
	return energy.0 / (ld * ld + 1.0);
}
...
void main(void)
{
	...
	float ld = spheres(pos) * 5.0;
	float latt = 2.0 / (ld * ld + 1.0);
	vec3 radiance = vec3(1.0, 0.0, 0.0) * latt + occ * 0.01;
